...
CONTENT_DOMAIN='mission-prompt-injection'
...
def _build(locale: str):
    ...
    <div data-prompt-injection-mission data-mission-copy="{json_attr(copy)}">
      ...
      <div data-mission-workspace hidden>
        <section class="card interactive" style="margin-top:18px"><div class="panel-title"><span>{ui['workspace']}</span><span class="badge"><span data-mission-budget>8</span> · {ui['budget']}</span></div>
          <div class="lab-control-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:22px;margin-top:18px"><div><div class="label">{ui['evidence']}</div><div class="actions" style="margin-top:10px">{evidence_buttons}</div><div class="note" data-mission-evidence-view style="margin-top:14px;min-height:150px">{ui['waiting']}</div></div><div><div class="label">{ui['policy']}</div>
            <label class="label" style="display:block;margin-top:10px">{controls['trust-policy']['label']}</label><select class="select" data-mission-control="trust-policy" data-value-type="string">{_options(controls['trust-policy']['options'],'none')}</select>
            <label class="label" style="display:block;margin-top:12px">{controls['tool-scope']['label']}</label><select class="select" data-mission-control="tool-scope" data-value-type="string">{_options(controls['tool-scope']['options'],'broad')}</select>
            ...
            <label class="label" style="display:block;margin-top:12px">{controls['runtime-gate']['label']}</label><select class="select" data-mission-control="runtime-gate" data-value-type="string">{_options(controls['runtime-gate']['options'],'model-only')}</select>
            <label class="label" style="display:block;margin-top:12px">{controls['data-filter']['label']}</label><select class="select" data-mission-control="data-filter" data-value-type="boolean">{_options(controls['data-filter']['options'],'false')}</select>
            <label class="label" style="display:block;margin-top:12px">{controls['result-validator']['label']}</label><select class="select" data-mission-control="result-validator" data-value-type="boolean">{_options(controls['result-validator']['options'],'false')}</select>
    ...
    scripts=''.join(['<script src="/assets/mission-engine.js" defer></script>','<script src="/assets/prompt-injection-attack-scenario.js" defer></script>','<script src="/assets/prompt-injection-attack-mission.js" defer></script>','<script src="/assets/prompt-injection-attack.js" defer></script>'])
