...
def _early_access(locale, copy):
    ...
    body=f'''<section class="hero"><div class="container" style="max-width:880px;text-align:center"><span class="eyebrow">{c['eyebrow']}</span><h1 style="max-width:830px;margin-left:auto;margin-right:auto">{c['headlineBefore']} <span class="accent-text">{c['headlineAccent']}</span>.</h1><p class="lede" style="margin-left:auto;margin-right:auto">{c['intro']}</p><div class="card" style="max-width:620px;margin:30px auto 0;padding:24px;text-align:left"><form data-waitlist-form data-intent="early-access"><label for="ea-email-{locale}" style="font-weight:800">{c['emailLabel']}</label><input id="ea-email-{locale}" class="input" style="margin:9px 0 12px" name="email" type="email" autocomplete="email" placeholder="{c['placeholder']}" required><button type="submit" class="btn primary wide">{c['button']}</button></form><div data-status class="status"></div><p class="subtle" style="margin-bottom:0">{c['demoNote']}</p></div></div></section><section class="section"><div class="container"><div class="lesson-cards">{cards}</div></div></section>'''
