...
assert.ok(page.includes('MissionRuntimeWorkspace'),'Final Boss page must mount the current React Mission workspace');
assert.ok(!page.includes('data-reliable-support-agent'),'Next Final Boss page must not mount the retired static Build adapter');
assert.ok(!page.includes('data-build-reference'),'Final Boss must not expose a reference-architecture cheat');
