...
const visibleAnswer=mission.inspectEvidence('answer').value;
assert.equal(visibleAnswer.sourceId,'refund-policy-v2');
assert.equal(visibleAnswer.claim,'30 days');
assert.equal(Object.hasOwn(visibleAnswer,'correct'),false,'evidence must not reveal correctness directly');
assert.equal(Object.hasOwn(visibleAnswer,'authoritativeAnswer'),false,'evidence must not reveal the ground-truth answer');
...
assert.equal(result.attempt.frame.derived.answerCorrect,false);
...
assert.equal(result.attempt.frame.derived.answerCorrect,false,'more top-k must not magically fix stale authority');
...
assert.equal(result.attempt.frame.derived.answerCorrect,true);
...
function viableFingerprint(){
  ...
  return {
    ...
    answerCorrect:run.attempt.frame.derived.answerCorrect,
...
console.log('PASS Broken RAG Pipeline: evidence-first baseline, more-context trap, viable evidence policy, compression trap, compare, release decision, deterministic replay.');
