#!/usr/bin/env python3
"""Audit current Practice evidence density after Core-120 publication."""
...
def audit():
    ...
    wave=set(plan['core120Additions'])
...
def check(a):
    ...
    tests=[(a['publishedGuideCount']==120,f"Expected 120 published Guides, got {a['publishedGuideCount']}"),(a['guidesWithPracticeLink']==120,f"Expected 120 Guides with Practice links, got {a['guidesWithPracticeLink']}"),(not a['invalidGuidePracticeLinks'],f"Invalid Guide Practice links: {a['invalidGuidePracticeLinks']}"),(p['experienceCount']==17,f"Expected 17 production Experiences, got {p['experienceCount']}"),(a['productionExperienceTypeCounts']==types,f"Production type distribution drifted: {a['productionExperienceTypeCounts']}"),(p['pathMemberships']==19 and p['pathReach']==15,'Production Practice plan must remain 19 memberships across 15 Paths'),(r['experienceCount']==21 and r['pathMemberships']==25 and r['pathReach']==15,'Reconciled evidence must remain 21 Practices / 25 memberships / 15 Paths'),(p['guideConceptsExplicitlyCovered']==57,f"Expected 57/120 Guide Concepts with production Practice evidence, got {p['guideConceptsExplicitlyCovered']}"),(p['samePathExplicitPracticeMemberships']==69,f"Expected 69/183 production same-Path explicit Practice memberships, got {p['samePathExplicitPracticeMemberships']}"),(p['waveSamePathExplicitPracticeMemberships']==8,f"Expected 8/20 Core-120 additions with production same-Path Practice evidence, got {p['waveSamePathExplicitPracticeMemberships']}"),(r['guideConceptsExplicitlyCovered']==80,f"Expected 80/120 Guide Concepts with reconciled Practice evidence, got {r['guideConceptsExplicitlyCovered']}"),(r['guidedPathConceptMemberships']==183,f"Expected 183 guided Path-Concept memberships, got {r['guidedPathConceptMemberships']}"),(r['samePathExplicitPracticeMemberships']==96,f"Expected 96/183 same-Path explicit Practice memberships, got {r['samePathExplicitPracticeMemberships']}"),(r['wavePathConceptMemberships']==20,f"Expected 20 Core-120 addition memberships, got {r['wavePathConceptMemberships']}"),(r['waveSamePathExplicitPracticeMemberships']==10,f"Expected 10/20 Core-120 additions with explicit same-Path Practice evidence, got {r['waveSamePathExplicitPracticeMemberships']}"),(a['monetization']['billingActivation'] is False and a['monetization']['freeChoiceActivation'] is False,'Practice audit must not activate monetization')]
    errors.extend(msg for ok,msg in tests if not ok); return errors

def report(a):
    p=a['productionOnly']; r=a['reconciled']; print('Practice Density Audit — Core-120 publication'); print(f"Guide links: {a['guidesWithPracticeLink']}/{a['publishedGuideCount']} | unique targets {a['uniqueGuidePracticeTargets']}"); print(f"Production-only: {p['guideConceptsExplicitlyCovered']}/120 Guide Concepts | same-Path {p['samePathExplicitPracticeMemberships']}/183 ({pct(p['samePathExplicitPracticeMemberships'],183)}) | Core-120 {p['waveSamePathExplicitPracticeMemberships']}/20"); print(f"Reconciled: {r['guideConceptsExplicitlyCovered']}/120 Guide Concepts | same-Path {r['samePathExplicitPracticeMemberships']}/183 ({pct(r['samePathExplicitPracticeMemberships'],183)}) | Core-120 {r['waveSamePathExplicitPracticeMemberships']}/20 ({pct(r['waveSamePathExplicitPracticeMemberships'],20)})"); print(); print('| Path | Guided | Explicitly practiced | Coverage |'); print('| --- | ---: | ---: | ---: |');
