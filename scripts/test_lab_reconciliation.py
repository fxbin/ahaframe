...
VALIDATION = ROOT / "web" / "lib" / "validation-context.ts"
...
def main() -> None:
    ...
    for item in experiences:
        ...
        pattern = rf'"{relative}"\s*:\s*\{{[^\n]*labId:\s*"{lab_id}"'
        assert re.search(pattern, validation), f"{item['id']}: stable route/labId missing from web/lib/validation-context.ts"
    ...
    print("PASS Lab reconciliation v0.8: 10 experiences classified, 4-step Campaign bounded, public routes/lab_ids preserved in the Next validation context, Mission identity additive.")
