(function(){
  ...
  controls.forEach((control)=>control.addEventListener('change',()=>{const id=control.dataset.missionControl;let value=control.value;if(control.dataset.valueType==='boolean')value=value==='true';try{mission.intervene(id,{value});status.textContent='';outcome.textContent=ui.waiting||'Run the attack suite.';renderBudget();window.AhaFrame?.track?.('instruction_conflict_parameter_changed',{missionId:'prompt-injection-attack',interventionId:id});}catch(error){status.textContent=error.message;syncControls(mission.getLabFrame());}}));
