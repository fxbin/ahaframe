(function(){
  ...
  controls.forEach((control)=>control.addEventListener('change',()=>{
    ...
    try{mission.intervene(id,{value});status.textContent='';outcome.textContent=ui.waiting||'Replay traffic.';renderBudget();window.AhaFrame?.track?.('agent_reliability_parameter_changed',{missionId:'47000-retry',interventionId:id});}catch(error){status.textContent=error.message;syncControls(mission.getLabFrame());}
  }));
