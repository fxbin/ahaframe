...
function copy(locale: KnowledgeMap["locale"]) {
  return locale === "en"
    ? {
        ...
        branches: "branches", concepts: "concepts", shared: "shared", guide: "Guide", coursesKicker: "Prefer a clear path?",
    ...
    : {
        ...
        branches: "个分支", concepts: "个知识点", shared: "跨路径复用", guide: "Guide", coursesKicker: "更想按顺序学？",
      ...
      <details className="border-t border-[var(--border)] py-4" data-branch-id={branch.id}>
        ...
        <div className="mt-4 space-y-1 pl-3 sm:pl-5">
          {concepts.map((concept) => {
            const body = <><span><span className="font-medium">{concept.title}</span><span className="ml-2 font-mono text-[9px] uppercase text-[var(--muted)]">{concept.kind}</span></span><span className="flex shrink-0 items-center gap-2 font-mono text-[9px]">{concept.branchIds.length > 1 ? <span className="text-[var(--muted)]">{labels.shared}</span> : null}{concept.guideSlug ? <span className="text-[var(--brand-accent)]">{labels.guide} →</span> : null}</span></>;
            return concept.guideSlug ? (
              <Link key={concept.id} href={`/${segment}/guides/${concept.guideSlug}/`} className="flex items-start justify-between gap-4 border-l border-[var(--brand-accent)] py-2 pl-4 text-sm transition hover:text-[var(--brand-accent)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--brand-accent)]" data-guide-concept-id={concept.id}>{body}</Link>
            ) : <div key={concept.id} className="flex items-start justify-between gap-4 border-l border-[var(--border)] py-2 pl-4 text-sm">{body}</div>;
          })}
