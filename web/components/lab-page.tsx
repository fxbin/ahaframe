...
export function LabPage({ locale, lab }: LabPageProps) {
  ...
  return (
    <main className="py-12 sm:py-16">
      <section className="shell">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
          <div className="rounded-[28px] border border-[var(--border)] bg-white p-6 sm:p-8">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--primary)]">Production Lab</div><h2 className="mt-1 text-2xl font-black tracking-[-0.04em]">{lab.name}</h2></div><span className="rounded-full bg-[var(--surface-soft)] px-3 py-1 text-xs text-[var(--muted)]">M2 static parity shell</span></div>
            ...
            <div className="mt-6 rounded-2xl border border-dashed border-[var(--primary)] bg-[color-mix(in_srgb,var(--primary)_5%,white)] p-4 text-sm leading-6 text-[var(--muted)]"><strong className="text-[var(--text)]">Platform M3 boundary:</strong> deterministic controls, presets, compare/reset, checkpoints, and failure-state behavior are intentionally not reimplemented in React. #28 will mount the existing framework-independent Lab Engine here.</div>
          </div>

          <aside className="rounded-[28px] bg-[var(--text)] p-6 text-white sm:p-8"><div className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--accent)]">In one sentence</div><p className="mt-4 text-xl font-semibold leading-8">{lab.quick}</p><div className="mt-8 text-xs font-bold uppercase tracking-[0.12em] text-white/45">Migration invariant</div><p className="mt-3 text-sm leading-6 text-white/65">Content and SEO render on the server. The simulation engine remains a separate deterministic runtime so framework migration does not rewrite the learning model.</p></aside>
        </div>
      </section>

      {lab.takeaways?.length ? <section className="shell mt-16"><h2 className="text-3xl font-black tracking-[-0.04em]">Key takeaways</h2><div className="mt-6 grid gap-3 md:grid-cols-2">{lab.takeaways.map(([title, body]) => <article key={title} className="rounded-[22px] border border-[var(--border)] bg-white p-5"><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{body}</p></article>)}</div></section> : null}
      ...
      {lab.guide?.faq?.length ? <section className="shell mt-16"><h2 className="text-3xl font-black tracking-[-0.04em]">Common questions</h2><div className="mt-6 divide-y divide-[var(--border)] rounded-[24px] border border-[var(--border)] bg-white">{lab.guide.faq.map((item) => <details key={item.q} className="p-5"><summary className="cursor-pointer font-bold">{item.q}</summary><p className="mt-3 max-w-4xl leading-7 text-[var(--muted)]">{item.a}</p></details>)}</div><p className="mt-4 text-xs leading-5 text-[var(--muted)]">{lab.guide.note}</p></section> : null}

      {(lab.challenge || lab.next) ? <section className="shell mt-16 grid gap-4 lg:grid-cols-2">{lab.challenge ? <div className="rounded-[26px] bg-[var(--surface-soft)] p-7"><div className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--primary)]">Build challenge</div><h2 className="mt-3 text-2xl font-black tracking-[-0.04em]">{lab.challenge.title}</h2><p className="mt-3 leading-7 text-[var(--muted)]">{lab.challenge.body}</p>{lab.challenge.href && lab.challenge.button ? <Link className="mt-5 inline-flex rounded-full bg-[var(--text)] px-5 py-3 text-sm font-bold text-white" href={`${localizedPath(lab.challenge.href, locale)}${lab.challenge.query ?? ""}`}>{lab.challenge.button}</Link> : null}</div> : <div />}{lab.next ? <div className="rounded-[26px] border border-[var(--border)] bg-white p-7"><h2 className="text-2xl font-black tracking-[-0.04em]">{lab.next.title}</h2><p className="mt-3 leading-7 text-[var(--muted)]">{lab.next.description}</p><Link className="mt-5 inline-flex rounded-full bg-[var(--text)] px-5 py-3 text-sm font-bold text-white" href={`${localizedPath(lab.next.href, locale)}${lab.next.query ?? ""}`}>{lab.next.button}</Link></div> : null}</section> : null}
    </main>
  );
}
