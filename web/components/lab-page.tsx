  ...
  return (
    <main className="py-12 sm:py-16">
      <section className="shell">
        <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted)]"><span>{lab.layer}</span><span>·</span><span>{lab.level}</span><span>·</span><span>{lab.minutes} {copy.minutes}</span></div>
      ...
      <ExperienceStage locale={locale} stage="experience">
        {specialist ? (
          <aside className="max-w-4xl border-l-2 border-[var(--accent)] pl-5">
          ...
          <div className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
            <div className="rounded-[28px] border border-[var(--border)] bg-white p-6 sm:p-8">
              <div><div className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--primary)]">{copy.productionLab}</div><h3 className="mt-1 text-2xl font-black tracking-[-0.04em]">{lab.name}</h3></div>
      ...
      <ExperienceStage locale={locale} stage="reflection">
        {lab.takeaways?.length ? <div><h3 className="text-2xl font-black tracking-[-0.04em]">{copy.keyTakeaways}</h3><div className="mt-6 grid gap-3 md:grid-cols-2">{lab.takeaways.map(([title, body]) => <article key={title} className="rounded-[22px] border border-[var(--border)] bg-white p-5"><h4 className="font-bold">{title}</h4><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{body}</p></article>)}</div></div> : null}
      </ExperienceStage>

      <ExperienceStage locale={locale} stage="learn-more">
        <div className="space-y-12">
          {lab.guide ? <div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr]"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--primary)]">{lab.guide.eyebrow ?? lab.layer}</p><h3 className="mt-3 text-2xl font-black tracking-[-0.04em]">{lab.guide.title}</h3><p className="mt-4 leading-7 text-[var(--muted)]">{lab.guide.intro}</p></div><div className="space-y-4">{lab.guide.sections.map((section) => <article key={section.title} className="rounded-[22px] border border-[var(--border)] bg-white p-6"><h4 className="text-lg font-bold">{section.title}</h4><p className="mt-2 leading-7 text-[var(--muted)]">{section.body}</p></article>)}</div></div> : null}

          {lab.guide?.faq?.length ? <div><h3 className="text-2xl font-black tracking-[-0.04em]">{copy.commonQuestions}</h3><div className="mt-6 divide-y divide-[var(--border)] rounded-[24px] border border-[var(--border)] bg-white">{lab.guide.faq.map((item) => <details key={item.q} className="p-5"><summary className="cursor-pointer font-bold">{item.q}</summary><p className="mt-3 max-w-4xl leading-7 text-[var(--muted)]">{item.a}</p></details>)}</div><p className="mt-4 text-xs leading-5 text-[var(--muted)]">{lab.guide.note}</p></div> : null}
      ...
      <ExperienceStage locale={locale} stage="next">
        {(lab.challenge || lab.next) ? <div className="grid gap-4 lg:grid-cols-2">{lab.challenge ? <div className="rounded-[26px] bg-[var(--surface-soft)] p-7"><div className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--primary)]">{copy.buildChallenge}</div><h3 className="mt-3 text-2xl font-black tracking-[-0.04em]">{lab.challenge.title}</h3><p className="mt-3 leading-7 text-[var(--muted)]">{lab.challenge.body}</p>{lab.challenge.href && lab.challenge.button ? <Link className="mt-5 inline-flex rounded-full bg-[var(--text)] px-5 py-3 text-sm font-bold text-white" href={`${localizedPath(lab.challenge.href, locale)}${lab.challenge.query ?? ""}`}>{lab.challenge.button}</Link> : null}</div> : <div />}{lab.next ? <div className="rounded-[26px] border border-[var(--border)] bg-white p-7"><h3 className="text-2xl font-black tracking-[-0.04em]">{lab.next.title}</h3><p className="mt-3 leading-7 text-[var(--muted)]">{lab.next.description}</p><Link className="mt-5 inline-flex rounded-full bg-[var(--text)] px-5 py-3 text-sm font-bold text-white" href={`${localizedPath(lab.next.href, locale)}${lab.next.query ?? ""}`}>{lab.next.button}</Link></div> : null}</div> : null}
      </ExperienceStage>
    </main>
  );
}
