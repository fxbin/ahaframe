...
export function GuideDirectoryPage({ locale, data, problemDiscovery }: GuideDirectoryPageProps) {
  ...
  function reset() {
    setQuery(""); setDomain(""); setBranch(""); setPathId(""); setDifficulty(""); setPracticeOnly(false);
  }

  return (
    <main className="editorial-guide-page" data-guide-directory>
      <section className="border-b border-[var(--border)] py-16 sm:py-24">
        <div className="shell max-w-5xl">
          ...
          <div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-[var(--muted)]"><strong className="text-[var(--text)]">{data.count} {labels.guides}</strong><span>·</span><Link className="quiet-link" href={`/${segment}/courses/`}>{labels.coursesLink}</Link></div>
        </div>
      </section>
        ...
        <div className="shell">
          <div className="grid gap-3 lg:grid-cols-[1.4fr_repeat(4,minmax(0,1fr))]">
            <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
              {labels.filter}
              <input className="min-h-11 border border-[var(--border)] bg-transparent px-3 text-sm font-normal normal-case tracking-normal text-[var(--text)] outline-none focus:border-[var(--brand-accent)]" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={labels.search} />
            </label>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
            <label className="flex cursor-pointer items-center gap-2 text-sm"><input type="checkbox" checked={practiceOnly} onChange={(event) => setPracticeOnly(event.target.checked)} />{labels.practiceOnly}</label>
            <div className="flex items-center gap-4 text-sm text-[var(--muted)]"><span data-guide-directory-count>{labels.showing} <strong className="text-[var(--text)]">{filtered.length}</strong> {labels.of} {data.count}</span>{active ? <button type="button" className="quiet-link" onClick={reset}>{labels.reset}</button> : null}</div>
          </div>
        </div>
      </section>

      <section className="py-12 sm:py-16"><div className="shell">
        {!filtered.length ? (
          <div className="mx-auto max-w-xl border-y border-[var(--border)] py-14 text-center" data-guide-directory-empty><p className="text-lg font-semibold">{labels.empty}</p><button type="button" className="editorial-text-link mt-4" onClick={reset}>{labels.emptyAction}</button></div>
          ...
          return (
            <section key={domainItem.id} className="border-t border-[var(--border)] py-10 sm:py-14" data-guide-directory-domain={domainItem.id}>
              <div className="grid gap-8 lg:grid-cols-[220px_1fr]">
                <div><h2 className="font-[family-name:var(--font-editorial)] text-2xl font-semibold tracking-[-0.035em]">{domainItem.title}</h2><p className="mt-2 text-sm text-[var(--muted)]">{items.length} {labels.guides}</p></div>
                <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
                  {items.map((item) => (
                    <Link key={item.slug} href={`/${segment}/guides/${item.slug}/`} className="group grid gap-4 py-6 sm:grid-cols-[1fr_auto] sm:items-center" data-guide-directory-item={item.slug}>
                      <span>
                        <span className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-[0.08em] text-[var(--muted)]"><span>{item.branchTitle}</span><span>·</span><span>{item.difficulty}</span><span>·</span><span>{item.readingMinutes} {labels.min}</span>{item.hasPractice ? <><span>·</span><span className="text-[var(--brand-accent)]">{labels.practice}</span></> : null}</span>
                        <strong className="mt-2 block font-[family-name:var(--font-editorial)] text-2xl font-semibold tracking-[-0.035em] group-hover:text-[var(--brand-accent)]">{item.title}</strong>
...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: Array<{ id: string; title: string }>; }) {
  return (
    <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-[var(--muted)]">
      {label}
      <select className="min-h-11 border border-[var(--border)] bg-[var(--background)] px-3 text-sm font-normal normal-case tracking-normal text-[var(--text)] outline-none focus:border-[var(--brand-accent)]" value={value} onChange={(event) => onChange(event.target.value)}>
        <option value="">{label}</option>{options.map((option) => <option key={option.id} value={option.id}>{option.title}</option>)}
      </select>
    </label>
  );
}
