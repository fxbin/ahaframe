    ...
    <details className="group relative" data-testid="locale-switch">
      ...
      <div className="absolute right-0 z-[70] mt-2 min-w-48 overflow-hidden rounded-[8px] border border-[var(--border)] bg-white p-1 shadow-[0_18px_50px_rgba(15,23,42,0.14)]">
        {LOCALES.map((option) => {
          ...
          const row = (
            <span className="flex w-full items-center justify-between gap-5 rounded-[6px] px-3 py-2.5 text-sm">
              <span>{labels[option]}</span>
              <span className="font-mono text-[10px] font-bold text-[var(--muted)]">{localeCode(option)}</span>
            </span>
          );

          return current ? (
            <span key={option} aria-current="true" className="block bg-[var(--surface-soft)] text-[var(--text)]">
              {row}
            </span>
          ) : (
            <Link
              key={option}
              className="block text-[var(--muted)] transition hover:bg-[var(--surface-soft)] hover:text-[var(--text)] focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--primary)]"
              href={`${targetPath(pathname, option)}${search}`}
              hrefLang={option}
              lang={option === "zh-CN" ? "zh-CN" : "en"}
            >
              {row}
            </Link>
          );
