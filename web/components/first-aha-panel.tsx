  ...
  return (
    <aside className="report-panel overflow-hidden" aria-labelledby="first-aha-title">
      <div className="border-b border-[var(--border)] px-5 py-4 sm:px-7">
        ...
        <h2 id="first-aha-title" className="mt-2 font-[family-name:var(--font-editorial)] text-2xl font-semibold tracking-[-0.035em] sm:text-[1.7rem]">{content.title}</h2>
      </div>
          ...
          <ol className="space-y-0" aria-label={content.title}>
            {content.trace.map((row) => (
              <li key={`${row.time}-${row.detail}`} className="grid grid-cols-[76px_70px_1fr] gap-3 border-b border-[var(--border)] py-3 font-mono text-[12px] leading-5 last:border-b-0 sm:grid-cols-[82px_82px_1fr] sm:text-[13px]">
                <time className="text-[var(--muted)]">{row.time}</time><span className="font-semibold text-[var(--muted)]">{row.actor}</span><span className={TRACE_STATE_CLASS[row.state]}>{row.detail}</span>
              </li>
            ))}
          </ol>
          <div className="incident-note mt-5 font-mono text-sm"><strong>{resultLabel}:</strong> {content.result}</div>
          <div className="mt-6 border-t border-[var(--border)] pt-5"><h3 className="font-[family-name:var(--font-editorial)] text-xl font-semibold leading-snug tracking-[-0.03em]">{content.insightTitle}</h3><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{content.insightCopy}</p></div>
        </div>
        ...
        <div className="p-5 sm:p-7">
          ...
          <div className="mt-4 space-y-2" role="group" aria-label={content.question}>
            {content.choices.map((choice) => {
              ...
              return (
                <button key={choice.id} type="button" aria-pressed={isSelected} data-event="first_aha_intervention_selected" data-choice={choice.id} onClick={() => setSelectedId(choice.id)} className={`w-full border px-4 py-3 text-left transition ${isSelected ? "border-[var(--brand-accent)] bg-[var(--brand-accent-soft)]" : "border-[var(--border)] bg-transparent hover:border-[var(--border-strong)]"}`}>
                  <span className="block text-sm font-bold">{choice.label}</span><span className="mt-1 block text-xs leading-5 text-[var(--muted)]">{choice.description}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-5 min-h-[108px] border-t border-[var(--border)] pt-5" aria-live="polite">
            {selected ? <div data-event="first_aha_consequence_observed"><span className={`inline-flex border px-2 py-1 font-mono text-[11px] font-bold ${TONE_CLASS[selected.tone]}`}>{selected.signal}</span><p className="mt-3 text-sm leading-6 text-[var(--muted)]">{selected.consequence}</p></div> : <p className="text-sm leading-6 text-[var(--muted)]">{idleCopy}</p>}
          </div>

          <Link className="primary-action mt-5" href={href} data-event="first_aha_cta_clicked">{ctaLabel} <span aria-hidden="true">→</span></Link>
        </div>
      </div>
    </aside>
  );
