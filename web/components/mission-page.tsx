  ...
  return (
    <main className="py-12 sm:py-16">
      ...
      <ExperienceStage locale={locale} stage="experience">
        <article className="rounded-[28px] bg-[var(--text)] p-7 text-white sm:p-9">
          <div className="text-xs font-extrabold uppercase tracking-[0.14em] text-[var(--accent)]">{mission.brief.eyebrow}</div>
          <h3 className="mt-3 max-w-4xl text-3xl font-black tracking-[-0.04em]">{mission.brief.title}</h3>
      ...
      <ExperienceStage locale={locale} stage="reflection">
        <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]">
          <div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--primary)]">{mission.debrief.eyebrow}</p><h3 className="mt-3 text-2xl font-black tracking-[-0.04em]">{mission.debrief.title}</h3><p className="mt-4 font-semibold leading-7">{mission.debrief.rule}</p></div>
        ...
        {mission.takeaways?.length ? <div className="mt-12"><h3 className="text-2xl font-black tracking-[-0.04em]">{copy.keyTakeaways}</h3><div className="mt-6 grid gap-3 md:grid-cols-3">{mission.takeaways.map(([title, body]) => <article key={title} className="rounded-[22px] border border-[var(--border)] bg-white p-5"><h4 className="font-bold">{title}</h4><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{body}</p></article>)}</div></div> : null}
      </ExperienceStage>

      <ExperienceStage locale={locale} stage="learn-more">
        <div className="space-y-12">
          {mission.incidentLedger?.length ? <div><h3 className="text-2xl font-black tracking-[-0.04em]">{copy.incidentLedger}</h3><div className="mt-6 grid gap-3 md:grid-cols-2">{mission.incidentLedger.map(([title, body]) => <article key={title} className="rounded-[22px] border border-[var(--border)] bg-white p-5"><h4 className="font-bold">{title}</h4><p className="mt-2 text-sm leading-6 text-[var(--muted)]">{body}</p></article>)}</div></div> : null}
      ...
      <ExperienceStage locale={locale} stage="next">
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-[26px] bg-[var(--surface-soft)] p-7"><h3 className="text-2xl font-black tracking-[-0.04em]">{mission.next.title}</h3><p className="mt-3 leading-7 text-[var(--muted)]">{mission.next.description}</p><Link className="mt-5 inline-flex rounded-full bg-[var(--text)] px-5 py-3 text-sm font-bold text-white" href={`${localizedPath(mission.next.href, locale)}${mission.next.query ?? ""}`}>{mission.next.button}</Link></div>
          {mission.earlyAccess ? <div className="rounded-[26px] border border-[var(--border)] bg-white p-7"><h3 className="text-2xl font-black tracking-[-0.04em]">{mission.earlyAccess.title}</h3><p className="mt-3 leading-7 text-[var(--muted)]">{mission.earlyAccess.description}</p><Link className="mt-5 inline-flex rounded-full border border-[var(--border)] px-5 py-3 text-sm font-bold" href={`${localizedPath(mission.earlyAccess.href, locale)}${mission.earlyAccess.query ?? ""}`}>{mission.earlyAccess.button}</Link></div> : null}
        </div>
      </ExperienceStage>
    </main>
  );
