...
import { getMissionContent, WAVE_2_BUILD_SLUGS } from "@/lib/mission";
...
export function generateStaticParams() {
  return WAVE_2_BUILD_SLUGS.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale: segment, slug } = await params;
  const locale = localeFromSegment(segment);
  if (!locale || !WAVE_2_BUILD_SLUGS.includes(slug as (typeof WAVE_2_BUILD_SLUGS)[number])) return {};
...
export default async function OutcomeBuildRoute({ params }: PageProps) {
  const { locale: segment, slug } = await params;
  const locale = localeFromSegment(segment);
  if (!locale || !WAVE_2_BUILD_SLUGS.includes(slug as (typeof WAVE_2_BUILD_SLUGS)[number])) notFound();
  const mission = await getMissionContent(locale, slug);
