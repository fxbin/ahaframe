...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = "AI Guides — AhaFrame";
  const description = locale === "zh-CN"
    ? `浏览 ${data.count} 个可复用的 AI 心智模型，按知识域、主题、课程、难度和 Practice 快速筛选。`
    : `Browse ${data.count} reusable AI mental models and filter by domain, topic, course, difficulty, and Practice availability.`;
...
export default async function GuidesRoute({ params }: PageProps) {
  ...
  const title = locale === "zh-CN" ? `${data.count} 个可复用的 AI 心智模型` : `${data.count} reusable mental models for AI`;
  const description = locale === "zh-CN"
