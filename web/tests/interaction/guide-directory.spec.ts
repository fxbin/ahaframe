...
test("English Guides directory exposes exactly 100 published Guides from canonical domains", async ({ page }) => {
  await page.goto("/en/guides/");

  await expect(page.getByRole("heading", { level: 1 })).toHaveText("100 reusable mental models for AI.");
...
test("directory filters stay deterministic and reset without crawlable filter URLs", async ({ page }) => {
  ...
  await expect(page.locator("[data-guide-directory-item]")).toHaveCount(100);
...
test("directory has an explicit zero-result state and can recover to all Guides", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Clear filters and view all 100" }).click();
  await expect(page.locator("[data-guide-directory-item]")).toHaveCount(100);
});

test("Chinese Guides directory preserves 100-guide parity and localized navigation", async ({ page }) => {
  await page.goto("/zh-cn/guides/");

  await expect(page.getByRole("heading", { level: 1 })).toHaveText("100 个可复用的 AI 心智模型。");
  await expect(page.locator("[data-guide-directory-item]")).toHaveCount(100);
