...
test("English homepage presents the six-step learning method and problem-first incidents", async ({ page }) => {
  ...
  const rag = page.locator('a[href="/en/labs/rag-failure/"]').filter({ hasText: "The Broken RAG Pipeline" });
  ...
  const injection = page.locator('a[href="/en/labs/instruction-conflict/"]').filter({ hasText: "The Prompt Injection Attack" });
...
test("Chinese homepage keeps the same learning method and curiosity hooks", async ({ page }) => {
  ...
  const rag = page.locator('a[href="/zh-cn/labs/rag-failure/"]').filter({ hasText: "失效的 RAG 管道" });
  await expect(rag).toContainText("正确文档明明已经被检索出来，Agent 为什么还是从错误证据里给出了答案？");

  const injection = page.locator('a[href="/zh-cn/labs/instruction-conflict/"]').filter({ hasText: "Prompt Injection 攻击" });
