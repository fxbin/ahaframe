...
const COPPER = "rgb(164, 95, 67)";
const LEGACY_GREEN = "#1f6654";

test.describe("editorial visual consistency", () => {
  test("Pricing and global search use the editorial copper accent", async ({ page }) => {
    ...
    await expect.poll(() => headlineAccent.evaluate((node) => getComputedStyle(node).color)).toBe(COPPER);

    const search = page.locator("[data-global-search-trigger]");
    await search.focus();
    await expect.poll(() => search.evaluate((node) => getComputedStyle(node).outlineColor)).toBe(COPPER);
  });
  ...
  test("Experience pages remap legacy runtime primary without changing semantic success", async ({ page }) => {
    ...
    await expect.poll(() => root.evaluate((node) => getComputedStyle(node).getPropertyValue("--primary").trim().toLowerCase())).toBe("#a45f43");
    await expect.poll(() => root.evaluate((node) => getComputedStyle(node).getPropertyValue("--success").trim().toLowerCase())).toBe("#2f7658");
  });

  test("Guide practice return bar is neutral paper with a copper editorial rule", async ({ page }) => {
    ...
    await expect.poll(() => inner.evaluate((node) => getComputedStyle(node).borderLeftColor)).toBe(COPPER);
  });
  ...
  test("Learning progress keeps semantic green instead of inheriting the brand accent", async ({ page }) => {
    ...
    await expect.poll(() => root.evaluate((node) => getComputedStyle(node).getPropertyValue("--brand-accent").trim().toLowerCase())).toBe("#a45f43");
  });
});
